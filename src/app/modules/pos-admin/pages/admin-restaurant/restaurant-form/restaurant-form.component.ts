...
export class RestaurantFormComponent implements OnInit {
  ...
  // Store the last valid country for the flag per phone input
  lastValidCountry: any[] = [];
  ...
  ngOnInit(): void {
    ...
    this.lastValidCountry = [this.countries[0] || null]; // Ensure lastValidCountry is initialized for first phone
  ...
  patchValuesToForm() {
    ...
    for (let j = 0; j < this.restaurantData?.phone.length; j++) {
      ...
      this.lastValidCountry[j] = matchedCountry; // Ensure lastValidCountry is set for edit, per input
  ...
  addMod(): void {
    ...
    const defaultCountry = this.countries[0] || null;
    this.countryCodeInput.push(defaultCountry?.dialCode || '');
    this.selectedCountry.push(defaultCountry);
    this.lastValidCountry.push(defaultCountry); // Ensure lastValidCountry is initialized for new phone
  ...
  removeMod(i) {
    ...
    this.lastValidCountry.splice(i, 1); // Remove lastValidCountry for removed phone
  }
  ...
  onCountryCodeChange(code: string, i: number, address: any) {
    // Only update flag if code is an exact match for a country's dial code
    const country = this.countries.find(c => c.dialCode === code.trim());
    if (country && code.trim() === country.dialCode) {
      // Update phone input and flag
      this.selectedCountry[i] = country;
      this.lastValidCountry[i] = country;
      ...
      // Reset phone input, keep flag as last valid (never set to null)
      this.selectedCountry[i] = this.lastValidCountry[i];
  ...
  // Returns true if any phone number input is showing the 'minimum digits required' error
  phoneNumbersHaveMinLengthError(): boolean {
    const phoneArray = this.resForm.get('phoneNo').controls;
    for (let i = 0; i < phoneArray.length; i++) {
      const numberControl = phoneArray[i].get('number');
      const country = this.selectedCountry[i];
      const minLength = country?.phoneLength || 0;
      if (numberControl.value && numberControl.value.length > 0 && numberControl.value.length < minLength) {
        return true;
      }
    }
    return false;
